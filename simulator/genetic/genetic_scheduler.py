        ...
        for workload, cluster in zip(self.workloads, schedule):
            if cluster == "wait":
                if workload["deadline"] <= 0:
                   deadline_penalty += 1000000 # severe penalty for not meeting deadline

                # decrease deadline penality, if it is better to run the job in the future
                # when the co2 intensity is lower
                # TODO 
